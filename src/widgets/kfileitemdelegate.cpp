...
public:
  ...
  QSize decorationSizeHint(const QStyleOptionViewItem &option,
                           const QModelIndex &index) const;
  QSize displaySizeHint(const QStyleOptionViewItem &option,
                        const QModelIndex &index) const;
  QString replaceNewlines(const QString &string) const;
  inline KFileItem fileItem(const QModelIndex &index) const;
  QString elidedText(QTextLayout &layout, const QStyleOptionViewItem &option,
  ...
  QSize layoutText(QTextLayout &layout, const QString &text,
                   int maxWidth) const;
  inline void setLayoutOptions(QTextLayout &layout,
                               const QStyleOptionViewItem &options) const;
  inline bool verticalLayout(const QStyleOptionViewItem &option) const;
  inline QBrush brush(const QVariant &value,
                      const QStyleOptionViewItem &option) const;
  QBrush foregroundBrush(const QStyleOptionViewItem &option,
                         const QModelIndex &index) const;
  inline void setActiveMargins(Qt::Orientation layout);
  void setVerticalMargin(MarginType type, int left, int right, int top,
                         int bottom);
  void setHorizontalMargin(MarginType type, int left, int right, int top,
                           int bottom);
  ...
  QString information(const QStyleOptionViewItem &option,
  ...
  QIcon decoration(const QStyleOptionViewItem &option,
                   const QModelIndex &index) const;
  QPoint iconPosition(const QStyleOptionViewItem &option) const;
  QRect labelRectangle(const QStyleOptionViewItem &option,
                       const QModelIndex &index) const;
  void layoutTextItems(const QStyleOptionViewItem &option,
                       const QModelIndex &index, QTextLayout *labelLayout,
                       QTextLayout *infoLayout, QRect *textBoundingRect) const;
  void drawTextItems(QPainter *painter, const QTextLayout &labelLayout,
                     const QColor &labelColor, const QTextLayout &infoLayout,
                     const QColor &infoColor,
                     const QRect &textBoundingRect) const;
  QPixmap applyHoverEffect(const QPixmap &icon) const;
  QPixmap transition(const QPixmap &from, const QPixmap &to,
                     qreal amount) const;
  ...
  void paintJobTransfers(QPainter *painter, const qreal &jobAnimationAngle,
...
KFileItemDelegate::Private::Private(KFileItemDelegate *parent)
    : shadowColor(Qt::transparent), shadowOffset(1, 1), shadowBlur(2),
      maximumSize(0, 0), showToolTipWhenElided(true),
      wrapMode(QTextOption::WrapAtWordBoundaryOrAnywhere),
      jobTransfersVisible(false), emblemRect(QRect()), activeMargins(nullptr) {
  Q_UNUSED(parent);
}

void KFileItemDelegate::Private::setActiveMargins(Qt::Orientation layout) {
  activeMargins =
      (layout == Qt::Horizontal ? horizontalMargin : verticalMargin);
}

void KFileItemDelegate::Private::setVerticalMargin(MarginType type, int left,
                                                   int top, int right,
                                                   int bottom) {
...
void KFileItemDelegate::Private::setHorizontalMargin(MarginType type, int left,
                                                     int top, int right,
                                                     int bottom) {
...
void KFileItemDelegate::Private::setVerticalMargin(MarginType type,
                                                   int horizontal,
                                                   int vertical) {
  setVerticalMargin(type, horizontal, vertical, horizontal, vertical);
}

void KFileItemDelegate::Private::setHorizontalMargin(MarginType type,
                                                     int horizontal,
                                                     int vertical) {
  setHorizontalMargin(type, horizontal, vertical, horizontal, vertical);
}

QRect KFileItemDelegate::Private::addMargin(const QRect &rect,
                                            MarginType type) const {
  Q_ASSERT(activeMargins != nullptr);
  const Margin &m = activeMargins[type];
  return rect.adjusted(-m.left, -m.top, m.right, m.bottom);
}

QRect KFileItemDelegate::Private::subtractMargin(const QRect &rect,
                                                 MarginType type) const {
  Q_ASSERT(activeMargins != nullptr);
  const Margin &m = activeMargins[type];
  return rect.adjusted(m.left, m.top, -m.right, -m.bottom);
...
QSize KFileItemDelegate::Private::addMargin(const QSize &size,
                                            MarginType type) const {
  Q_ASSERT(activeMargins != nullptr);
  const Margin &m = activeMargins[type];
...
QSize KFileItemDelegate::Private::subtractMargin(const QSize &size,
                                                 MarginType type) const {
  Q_ASSERT(activeMargins != nullptr);
  const Margin &m = activeMargins[type];
  return QSize(size.width() - m.left - m.right,
...
// Returns the size of a file, or the number of items in a directory, as a
// QString
QString KFileItemDelegate::Private::itemSize(const QModelIndex &index,
  ...
  const int count = value.typeId() == QMetaType::Int
                        ? value.toInt()
...
// Returns the additional information string, if one should be shown, or an
// empty string otherwise
QString
KFileItemDelegate::Private::information(const QStyleOptionViewItem &option,
  ...
  for (KFileItemDelegate::Information info : informationList) {
    ...
    case KFileItemDelegate::MimeType:
      string += item.isMimeTypeKnown() ? item.mimetype()
                                       : i18nc("@info mimetype", "Unknown");
      break;

    case KFileItemDelegate::FriendlyMimeType:
      string += item.isMimeTypeKnown() ? item.mimeComment()
                                       : i18nc("@info mimetype", "Unknown");
      break;
...
// Replaces any newline characters in the provided string, with
// QChar::LineSeparator
...
// Elides the text in the layout, by iterating over each line in the layout,
// eliding or word breaking the line if it's wider than the max width, and
// finally adding an ellipses at the end of the last line, if there are more
// lines than will fit within the vertical size constraints.
QString
KFileItemDelegate::Private::elidedText(QTextLayout &layout,
                                       const QStyleOptionViewItem &option,
  ...
  for (int i = 0; i < layout.lineCount(); i++) {
    ...
    if (height + line.height() + option.fontMetrics.lineSpacing() > maxHeight) {
      ...
      // elidedText() will be too clever and keep adding lines until it finds
      // one that's too wide.
      if (line.naturalTextWidth() < maxWidth &&
        ...
        elided += option.fontMetrics.elidedText(text.mid(start),
                                                option.textElideMode, maxWidth);
      }
      break;
    } else if (line.naturalTextWidth() > maxWidth) {
      elided += option.fontMetrics.elidedText(text.mid(start, length),
                                              option.textElideMode, maxWidth);
...
void KFileItemDelegate::Private::setLayoutOptions(
    QTextLayout &layout, const QStyleOptionViewItem &option) const {
...
QSize KFileItemDelegate::Private::displaySizeHint(
    const QStyleOptionViewItem &option, const QModelIndex &index) const {
  QString label = option.text;
  int maxWidth = 0;
  if (maximumSize.isEmpty()) {
    maxWidth = verticalLayout(option) &&
    ...
    maxWidth = maximumSize.width() - (itemMargin.left + itemMargin.right) -
...
QSize KFileItemDelegate::Private::decorationSizeHint(
    const QStyleOptionViewItem &option, const QModelIndex &index) const {
  ...
  if (!verticalLayout(option)) {
    ...
    iconSize.rwidth() =
        qMin(iconSize.width() + 10, option.decorationSize.width());
  }
...
bool KFileItemDelegate::Private::verticalLayout(
    const QStyleOptionViewItem &option) const {
...
QBrush
KFileItemDelegate::Private::brush(const QVariant &value,
                                  const QStyleOptionViewItem &option) const {
...
QBrush
KFileItemDelegate::Private::foregroundBrush(const QStyleOptionViewItem &option,
                                            const QModelIndex &index) const {
...
bool KFileItemDelegate::Private::isListView(
    const QStyleOptionViewItem &option) const {
...
QPixmap
KFileItemDelegate::Private::applyHoverEffect(const QPixmap &icon) const {
  QPixmap result = icon;
  KIconEffect::toActive(result);
  return result;
}

QPixmap KFileItemDelegate::Private::transition(const QPixmap &from,
                                               const QPixmap &to,
                                               qreal amount) const {
...
// FIXME: Somehow this doesn't work on Mac OS..
  ...
  const bool usePixmap =
      from.paintEngine()->hasFeature(QPaintEngine::PorterDuff) &&
      from.paintEngine()->hasFeature(QPaintEngine::BlendModes);
#endif

  // If the native paint engine supports Porter/Duff compositing and
...
void KFileItemDelegate::Private::layoutTextItems(
  ...
  // If we have additional info text, and there's space for at least two lines
  // of text, adjust the max label size to make room for at least one line of
  // the info text
  if (!info.isEmpty() &&
      textRect.height() >= option.fontMetrics.lineSpacing() * 2) {
  ...
  // Lay out the label text, and adjust the max info size based on the label
  // size
  ...
  const QSize size(qMax(labelSize.width(), infoSize.width()),
                   labelSize.height() + infoSize.height());
  *textBoundingRect = QStyle::alignedRect(
      option.direction, option.displayAlignment, size, textRect);

  // Compute the positions where we should draw the layouts
  labelLayout->setPosition(QPointF(textRect.x(), textBoundingRect->y()));
  infoLayout->setPosition(
      QPointF(textRect.x(), textBoundingRect->y() + labelSize.height()));
}

void KFileItemDelegate::Private::drawTextItems(
    QPainter *painter, const QTextLayout &labelLayout, const QColor &labelColor,
    const QTextLayout &infoLayout, const QColor &infoColor,
  ...
  if (shadowColor.alpha() > 0) {
    ...
    QImage image(boundingRect.size() + QSize(padding * 2, padding * 2),
    ...
    painter->drawImage(boundingRect.topLeft() - QPoint(padding, padding) +
                           shadowOffset.toPoint(),
                       image);
...
void KFileItemDelegate::Private::initStyleOption(
...
void KFileItemDelegate::Private::paintJobTransfers(
    QPainter *painter, const qreal &jobAnimationAngle, const QPoint &iconPos,
    const QStyleOptionViewItem &opt) {
  painter->save();
  QSize iconSize = opt.icon.actualSize(opt.decorationSize);
  QPixmap downArrow = downArrowIcon.pixmap(iconSize * 0.30);
  // corner (less x and y than bottom-right corner) that we will center the
  // painter around
  QPoint bottomRightCorner = QPoint(iconPos.x() + iconSize.width() * 0.75,
                                    iconPos.y() + iconSize.height() * 0.60);
  ...
  painter->drawPixmap(-downArrow.size().width() * .50,
                      -downArrow.size().height() * .50, downArrow);
...
KFileItemDelegate::KFileItemDelegate(QObject *parent)
  ...
  d->setHorizontalMargin(Private::ItemMargin, focusHMargin, 0);

  // Margins for vertical mode (icon views)
  d->setVerticalMargin(Private::TextMargin, focusHMargin, 0);
...
QSize KFileItemDelegate::sizeHint(const QStyleOptionViewItem &option,
  ...
  if (d->verticalLayout(opt)) {
    size.rwidth() =
        qMax(displaySize.width(), decorationSize.width()) + hSpacing;
    size.rheight() =
        decorationSize.height() + displaySize.height() + 1 + vSpacing;
  } else {
  ...
  size =
      style->sizeFromContents(QStyle::CT_ItemViewItem, &opt, size, opt.widget);
...
QColor KFileItemDelegate::shadowColor() const { return d->shadowColor; }

void KFileItemDelegate::setShadowOffset(const QPointF &offset) {
  d->shadowOffset = offset;
}

QPointF KFileItemDelegate::shadowOffset() const { return d->shadowOffset; }

void KFileItemDelegate::setShadowBlur(qreal factor) { d->shadowBlur = factor; }

qreal KFileItemDelegate::shadowBlur() const { return d->shadowBlur; }

void KFileItemDelegate::setMaximumSize(const QSize &size) {
  d->maximumSize = size;
}

QSize KFileItemDelegate::maximumSize() const { return d->maximumSize; }
...
QIcon KFileItemDelegate::Private::decoration(const QStyleOptionViewItem &option,
                                             const QModelIndex &index) const {
...
QRect KFileItemDelegate::Private::labelRectangle(
    const QStyleOptionViewItem &option, const QModelIndex &index) const {
  QStyle *style =
...
QPoint KFileItemDelegate::Private::iconPosition(
    const QStyleOptionViewItem &option) const {
  QStyle *style =
      option.widget ? option.widget->style() : QApplication::style();
...
void KFileItemDelegate::Private::drawFocusRect(
  ...
  opt.rect = rect;
  opt.state =
      option.state | QStyle::State_KeyboardFocusChange | QStyle::State_Item;
  opt.backgroundColor = option.palette.color(
      option.state & QStyle::State_Selected ? QPalette::Highlight
                                            : QPalette::Base);
...
void KFileItemDelegate::paint(QPainter *painter,
  ...
  d->drawTextItems(painter, labelLayout, labelColor, infoLayout, infoColor,
                   textBoundingRect);
...
void KFileItemDelegate::drawSelectionEmblem(QStyleOptionViewItem option,
  ...
  if ((state & QStyle::State_MouseOver && !fileItem(index).isDir()) ||
    ...
    const QString selectionEmblem = state & QStyle::State_Selected
                                        ? QStringLiteral("emblem-remove")
                                        : QStringLiteral("emblem-added");
    const auto emblem =
        QIcon::fromTheme(selectionEmblem)
            .pixmap(d->emblemRect.size(), state & QStyle::State_MouseOver
                                              ? QIcon::Active
                                              : QIcon::Disabled);
...
bool KFileItemDelegate::editorEvent(QEvent *event, QAbstractItemModel *model,
                                    const QStyleOptionViewItem &option,
                                    const QModelIndex &index) {
  Q_UNUSED(event)
  Q_UNUSED(model)
...
void KFileItemDelegate::setModelData(QWidget *editor, QAbstractItemModel *model,
                                     const QModelIndex &index) const {
  QTextEdit *textedit = qobject_cast<QTextEdit *>(editor);
  Q_ASSERT(textedit != nullptr);
...
void KFileItemDelegate::updateEditorGeometry(QWidget *editor,
  ...
  if (!d->maximumSize.isEmpty()) {
    if (d->verticalLayout(option)) {
      ...
      int diff =
          qMax(r.width(), d->maximumSize.width() - opt.decorationSize.width()) -
          r.width();
      if (diff > 0) {
...
bool KFileItemDelegate::helpEvent(QHelpEvent *event, QAbstractItemView *view,
                                  const QStyleOptionViewItem &option,
                                  const QModelIndex &index) {
  Q_UNUSED(event)
  Q_UNUSED(view)

  // if the tooltip information the model keeps is different from the display
  // information, show it always
  ...
  const QString elidedText =
      d->elidedText(labelLayout, opt, textBoundingRect.size());
...
void KFileItemDelegate::setSelectionEmblemRect(QRect rect, int iconSize) {
  ...
  if (iconSize <= KIconLoader::SizeSmallMedium) {
    d->emblemRect =
        QRect(rect.topLeft().x() + emblemSize / 4,
              rect.topLeft().y() + emblemSize / 4, emblemSize, emblemSize);
  } else {
    d->emblemRect =
        QRect(rect.topLeft().x(), rect.topLeft().y(), emblemSize, emblemSize);
  }
}

QRect KFileItemDelegate::selectionEmblemRect() const { return d->emblemRect; }
